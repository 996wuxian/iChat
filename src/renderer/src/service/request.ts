...
service.interceptors.response.use(
  ...
  (error) => {
    ...
    // 直接使用已定义的状态码处理器
    if (responseHandlers[status]) {
      responseHandlers[status]()
      return Promise.reject(error)
    }

    const handlerKey = error?.status?.toString().slice(0, 1) || 'default'
    const handler = responseHandlers[handlerKey] || responseHandlers.default
    handler(error.response?.data?.message || error.response?.data?.msg)
    return Promise.reject(error)
  }
)
